...
import { emojiLog, formatForLog, isSourceFile, readTsconfig, relativePosix, removeExtension, toPosix, } from "./utils.js";
...
Examples:
  ...
  zshy --dry-run                          # Preview changes without writing files
    ...
    if (failThreshold !== "never" && failThreshold !== "warn" && failThreshold !== "error") {
        emojiLog("❌", `Invalid value for --fail-threshold: "${failThreshold}". Valid values are "never", "warn", or "error"`, "error");
        process.exit(1);
    }
    ...
    if (typeof pkgJson[CONFIG_KEY] === "string") {
        ...
        if (rawConfig.ignore !== undefined) {
            ...
            if (invalidPatterns.length > 0) {
                emojiLog("❌", `Invalid "ignore" patterns in package.json#/${CONFIG_KEY}: patterns must be non-empty relative globs without negations ("!"), absolute paths, or ".." segments. Invalid: ${invalidPatterns.join(", ")}`, "error");
                process.exit(1);
            }
        }
    }
    ...
    if (args["--project"]) {
        ...
        if (fs.existsSync(resolvedProjectPath)) {
            if (fs.statSync(resolvedProjectPath).isDirectory()) {
                emojiLog("❌", `--project must point to a tsconfig.json file, not a directory: ${resolvedProjectPath}`, "error");
                process.exit(1);
            }
        ...
        if (fs.existsSync(resolvedProjectPath)) {
            if (fs.statSync(resolvedProjectPath).isDirectory()) {
                emojiLog("❌", `zshy.tsconfig must point to a tsconfig.json file, not a directory: ${resolvedProjectPath}`, "error");
                process.exit(1);
            }
            tsconfigPath = resolvedProjectPath;
        }
        ...
        const foundTsconfigPath = await findUp(async (directory) => {
        ...
        if (foundTsconfigPath) {
            ...
            emojiLog("❌", `tsconfig.json not found when searching from ${pkgJsonDir} up to the git repository root`, "error");
            process.exit(1);
        }
    }
    else {
    ...
    if (relOutDir === "") {
        if (!pkgJson.files) {
            emojiLog("⚠️", 'You\'re building your code to the project root. This means your compiled files will be generated alongside your source files.\n   ➜ Setting "files" in package.json to exclude TypeScript source from the published package.');
   ...
   Ensure that your "files" in package.json excludes TypeScript source files, or your users may experience .d.ts resolution issues in some environments:
    ...
    if (isTypeModule) {
        ...
        emojiLog("🐢", `Package is a CommonJS module (${pkgJson.type === "commonjs" ? 'package.json#/type is "commonjs"' : 'package.json#/type not set to "module"'})`);
    }
    //////////////////////////////////////////////
    ...
    if (buildContext.errorCount > 0 || buildContext.warningCount > 0) {
        emojiLog("📊", `Compilation finished with ${buildContext.errorCount} error${buildContext.errorCount === 1 ? "" : "s"} and ${buildContext.warningCount} warning${buildContext.warningCount === 1 ? "" : "s"}`);
