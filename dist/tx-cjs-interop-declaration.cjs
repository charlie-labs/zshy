...
exports.createCjsInteropDeclarationTransformer = void 0;
const ts = __importStar(require("typescript"));
const tx_analyze_exports_js_1 = require("./tx-analyze-exports.cjs");
const createCjsInteropDeclarationTransformer = () => (context) => {
            ...
            if (stmt === defaultExportNode) {
                ...
                if (ts.isExportAssignment(stmt)) {
                    ...
                    if (ts.isIdentifier(stmt.expression)) {
                        ...
                        outStmts.push(factory.createVariableStatement(undefined, factory.createVariableDeclarationList([factory.createVariableDeclaration(factory.createIdentifier("_default"), undefined, undefined)], ts.NodeFlags.Const)));
                ...
                // case B: `export default function|class|interface|type|enum …`
                    ...
                    let name = ts.isFunctionDeclaration(stmt) ||
                    ...
                    const modifiers = (ts.canHaveModifiers(stmt) ? ts.getModifiers(stmt) : undefined)?.filter((m) => m.kind !== ts.SyntaxKind.ExportKeyword && m.kind !== ts.SyntaxKind.DefaultKeyword);
                    ...
                    if (ts.isFunctionDeclaration(stmt)) {
                        decl = factory.createFunctionDeclaration(declareModifiers, stmt.asteriskToken, name, stmt.typeParameters, stmt.parameters, stmt.type, undefined);
                    }
                    else if (ts.isClassDeclaration(stmt)) {
                        decl = factory.createClassDeclaration(declareModifiers, name, stmt.typeParameters, stmt.heritageClauses, stmt.members);
                    }
                    else if (ts.isInterfaceDeclaration(stmt)) {
                        decl = factory.createInterfaceDeclaration(declareModifiers, name, stmt.typeParameters, stmt.heritageClauses, stmt.members);
                    }
...
exports.createCjsInteropDeclarationTransformer = createCjsInteropDeclarationTransformer;
//# sourceMappingURL=tx-cjs-interop-declaration.js.map