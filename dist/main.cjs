...
Examples:
  ...
  zshy --dry-run                          # Preview changes without writing files
    ...
    if (typeof pkgJson[CONFIG_KEY] === "string") {
        ...
        if (rawConfig.ignore !== undefined) {
            ...
            if (invalidPatterns.length > 0) {
                (0, utils_js_1.emojiLog)("❌", `Invalid "ignore" patterns in package.json#/${CONFIG_KEY}: patterns must be non-empty relative globs without negations ("!"), absolute paths, or ".." segments. Invalid: ${invalidPatterns.join(", ")}`, "error");
                process.exit(1);
            }
        }
    }
    ...
    if (Array.isArray(config.ignore)) {
        const normalized = Array.from(new Set(config.ignore
            .map((p) => p.trim())
            .filter((p) => p.length > 0)
            .map(utils_js_1.toPosix)));
   ...
   Ensure that your "files" in package.json excludes TypeScript source files, or your users may experience .d.ts resolution issues in some environments:
    ...
    for (const [exportPath, sourcePath] of Object.entries(config.exports ?? {})) {
        ...
        if (typeof sourcePath === "string") {
            if (sourcePath.includes("*")) {
                ...
                const wildcardFiles = await (0, fast_glob_1.default)(pattern, {
    ...
    if (config.ignore && config.ignore.length > 0) {
        ...
        for (const patt of config.ignore) {
            const matches = await (0, fast_glob_1.default)(patt, { cwd: pkgJsonDir });
            for (const m of matches)
                ignoredSet.add((0, utils_js_1.toPosix)(m));
        }
        const before = entryPoints.length;
        const filtered = entryPoints.filter((ep) => {
            const rel = (0, utils_js_1.toPosix)(path.relative(pkgJsonDir, path.resolve(pkgJsonDir, ep)));
            return !ignoredSet.has(rel);
        });
        if (filtered.length !== before) {
            const removed = before - filtered.length;
            (0, utils_js_1.emojiLog)("⚠️", `Ignoring ${removed} file${removed === 1 ? "" : "s"} via zshy.ignore`);
        }
    ...
    if (assetEntrypoints.length > 0) {
        (0, utils_js_1.emojiLog)("📄", `${prefix}Copying ${assetEntrypoints.length} asset${assetEntrypoints.length === 1 ? "" : "s"}...`);
        for (const { sourcePath } of assetEntrypoints) {
            ...
            if (!fs.existsSync(sourceFile)) {
                (0, utils_js_1.emojiLog)("⚠️", `Asset not found: ${sourcePath}`, "warn");
                continue;
            }
            if (!isDryRun) {
            ...
            buildContext.copiedAssets.add((0, utils_js_1.toPosix)(path.relative(pkgJsonDir, destFile)));
            if (isVerbose) {
                const relativeSource = (0, utils_js_1.toPosix)(path.relative(pkgJsonDir, sourceFile));
                const relativeDest = (0, utils_js_1.toPosix)(path.relative(pkgJsonDir, destFile));
                (0, utils_js_1.emojiLog)("📋", `${isDryRun ? "[dryrun] " : ""}Copied asset: ./${relativeSource} → ./${relativeDest}`);
            }
        }
    }
    ///////////////////////////////////
    ...
    if (config.noEdit) {
        ...
        if (config.exports) {
            ...
            for (const { exportPath, sourcePath } of assetEntrypoints) {
                ...
                const relAssetPath = "./" + (0, utils_js_1.relativePosix)(pkgJsonDir, absAssetPath);
                ...
                if (exportPath === ".") {
                    ...
                    if (isVerbose) {
                        (0, utils_js_1.emojiLog)("🔧", `Setting "main": ${(0, utils_js_1.formatForLog)(relAssetPath)}`);
                        (0, utils_js_1.emojiLog)("🔧", `Setting "module": ${(0, utils_js_1.formatForLog)(relAssetPath)}`);
                        (0, utils_js_1.emojiLog)("🔧", `Setting "types": ${(0, utils_js_1.formatForLog)(relAssetPath)}`);
                    }
                }
            }
    ...
    if (buildContext.errorCount > 0 || buildContext.warningCount > 0) {
        (0, utils_js_1.emojiLog)("📊", `Compilation finished with ${buildContext.errorCount} error${buildContext.errorCount === 1 ? "" : "s"} and ${buildContext.warningCount} warning${buildContext.warningCount === 1 ? "" : "s"}`);
